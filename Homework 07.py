...
'''
PROBLEM 0. Oscar Ojeda Perez, Homework 07.
'''
...
def litCricFriend(wordList, text):
    ...
    lowerCaseText = lowerCaseText.replace('-', ' ') # Keep in ice to change for what they asked in this case --
    ...
    listOfWords = lowerCaseText.split()
    ...
    for listWords in listOfWords:
        words_list.append(listWords.strip(string.punctuation))
    ...
    ratio = 0
    num_words_text = len(words_list)
    ratio = countOfWords / num_words_text
...
print('cantoXIIAAnFrequency', litCricFriend(['a', 'an'], cantoXII))
print('bellsAAnFrequency', litCricFriend(['the'], theBells))
print('cantoXIIAAnFrequency', litCricFriend(['the'], cantoXII))
...
Yes, the results show that Poe and Neruda use 'a' and
'an' and 'the' differently because the ratio shows that
there is a drastic difference between the poems for both
i.e., use of 'a' and 'an' and 'the'.
'''
